...
namespace System.Windows.Data
{
    ...
    public sealed class PagedCollectionView : ICollectionView, IPagedCollectionView, IEditableCollectionView, INotifyPropertyChanged
    {
        ...
        /// </summary>
        private static object newItemPlaceholder = new object();
        ...
        /// CollectionChanged event ourselves, then we do not need to re-fire the event
        /// when it is handled by the Refresh operation after updating the snapshot.
        /// </summary>
        private bool _collectionChangeHandled;
        ...
        public PagedCollectionView(IEnumerable source) : this(source, false /*isDataSorted*/, false /*isDataInGroupOrder*/)
        {
        }
        ...
        public PagedCollectionView(IEnumerable source, bool isDataSorted, bool isDataInGroupOrder)
        {
            ...
            this._elementType = GetItemType(source);
            ...
            // if the source is an IList
            if (this.SourceList != null)
            {
                this._snapshot = this.SourceList;
                this.CopySnapshotToInternalList();
                this._trackingEnumerator = source.GetEnumerator();
            }
            else
            {
                this._snapshot = new List<object>();
                this.CopySnapshotToInternalList();
            ...
            {
                this.CanChangePage = this.PagedSource.CanChangePage;
                this.ItemCount = this.PagedSource.ItemCount;
                this.TotalItemCount = this.PagedSource.TotalItemCount;
                this.PagedSource.PageChanged += new EventHandler<PageChangedEventArgs>(this.PagedSource_PageChanged);
                INotifyPropertyChanged notifyingObject = this._sourceCollection as INotifyPropertyChanged;
                if (notifyingObject != null)
                {
                    notifyingObject.PropertyChanged += new PropertyChangedEventHandler(this.SourceCollection_PropertyChanged);
                }
            }
        ...
        private enum CollectionViewFlags
        {
            ...
            /// </summary>
            IsMoveToFirstPageDeferred = 0x100
        }
        ...
        public bool CanAddNew
        {
            get
            {
                return !this.IsEditingItem &&
                    ((this.SourceEditableCollection != null && this.SourceEditableCollection.CanAdd) ||
        ...
        public bool CanCancelEdit
        {
            get
            {
                return this.SourceEditableCollection != null ||
                    this._editItem is System.ComponentModel.IEditableObject;
            }
        }
        ...
        public bool CanFilter
        {
            get
            {
                // Local filtering needs to be turned off when the collection view is
                // used in the context of remote paging.
                return !this.IsSourceCollectionPaged;
            }
        }
        ...
        public bool CanRemove
        {
            get
            {
                return !this.IsEditingItem && !this.IsAddingNew &&
                    ((this.SourceEditableCollection != null && this.SourceEditableCollection.CanRemove) ||
        ...
        public int Count
        {
            get
            {
                ...
                // add a delta for the NewItemPlaceholder
                int delta = (NewItemPlaceholderPosition == NewItemPlaceholderPosition.None) ? 0 : 1;

                // if we have paging
                if (this.PageSize > 0)
                {
                    // if we have server paging, and we have not loaded the new data yet.
                    if (this.StartPageIndex > this.PageIndex)
                    {
                        return 0;
                    }
                    else
                    {
                        return Math.Max(0, Math.Min(this.PageSize + delta, this.InternalCount - (this._pageSize * this.LocalPageIndex) + delta));
                    }
                }
                else
                {
                    return delta + this.InternalCount;
                }
            }
        }
        ...
        /// </summary>
        private bool IsSourceCollectionPaged
        {
            get
            {
                return this.PagedSource != null;
            }
        }
        ...
        public int ItemCount
        {
            get
            {
                if (this.IsSourceCollectionPaged)
                {
                    // Remote-paging scenario. This IPagedCollection implementation owns this value.
                    return this._itemCount;
                }
                else
                {
                    // Local-paging scenario. This PagedCollectionView owns this value.
        ...
        public NewItemPlaceholderPosition NewItemPlaceholderPosition
        {
            ...
            set
            {
                if (value == this._newItemPlaceholderPosition)
                {
                    return;
                }
                ...
                // If we are currently in the middle of a "add new" transaction
                // and a new NewItemPlaceholderPosition is specified, throw an exception.
                if (this.IsAddingNew)
                {
                    throw new InvalidOperationException(string.Format(CultureInfo.InvariantCulture, PagedCollectionViewResources.OperationNotAllowedDuringTransaction, "NewItemPlaceholderPosition", "AddNew"));
                }

                NewItemPlaceholderPosition oldPosition = this._newItemPlaceholderPosition;

                NotifyCollectionChangedEventArgs removeArgs = new NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction.Remove, NewItemPlaceholder, -1);
                NotifyCollectionChangedEventArgs addArgs = new NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction.Add, NewItemPlaceholder, -1);

                // now make the change and raise the events
                this._newItemPlaceholderPosition = value;

                // If we moved the NewItemPlaceholderPosition, it will fire both a 'Remove' and
                // an 'Add'. If it used to be NewItemPlaceholderPosition.None and it changed,
                // then we only fire the 'Add'. If it changed to NewItemPlaceholderPosition.None,
                // we just call 'Remove'
                if (oldPosition != NewItemPlaceholderPosition.None)
                {
                    this.OnCollectionChanged(removeArgs);
                }
                if (value != NewItemPlaceholderPosition.None)
                {
                    this.OnCollectionChanged(addArgs);
                }

                this.AdjustCurrencyForNewItemPlaceholderMove();
        ...
        public int PageSize
        {
            ...
            set
            {
                ...
                if (this._pageSize != value)
                {
                    ...
                    IPagedCollection pagedCollection = this.PagedSource;
                    if (pagedCollection != null)
                    {
                        pagedCollection.PageSize = this._pageSize;
                    }
        ...
        {
            get { return this.PageIndex - this.StartPageIndex; }
        }
        ...
        private bool OnLastLocalPage
        {
            get
            {
                ...
                // if we have no items (PageCount==0) or there is just one page
                if (this.PageCount <= 1)
                {
                    return true;
                }

                return (this.PageIndex == this.StartPageIndex + this.PageCount - 1);
            }
        }
        ...
        {
            get { return (this._pageSize > 0) ? (int)Math.Ceiling((double)this.ItemCount / this._pageSize) : 0; }
        }

        /// <summary>
        /// Gets the SourceCollection as an IList
        /// </summary>
        private IPagedCollection PagedSource
        {
            get { return this.SourceCollection as IPagedCollection; }
        }
        ...
        /// </summary>
        private IEditableCollection SourceEditableCollection
        {
            get { return this.SourceCollection as IEditableCollection; }
        }
        ...
        {
            //// For example, if the data source has 1000 items, PageSize is 10, 
            //// and PageIndex goes from 0 to 20, the implementer of IPagedCollection could
            //// set StartPageIndex for example to 20. This means that _sourceCollection[0] 
            //// corresponds to index 200 within the 1000 items.
            get
            {
                IPagedCollection pagedCollection = this.PagedSource;
                return pagedCollection == null ? 0 : pagedCollection.StartPageIndex;
            }
        }
        ...
        public object AddNew()
        {
            ...
            Debug.Assert(!(this._itemConstructor != null && this.SourceEditableCollection != null), "_itemConstructor and SourceEditableCollection should never be set at the same time");

            if (this._itemConstructor != null)
            {
                newItem = this._itemConstructor.Invoke(null);
            }
            else if (this.SourceEditableCollection != null)
            {
                newItem = this.SourceEditableCollection.CreateNew();
            }
            ...
            try
            {
                ...
                if (this.SourceList != null)
                {
                    index = this.SourceList.Add(newItem);
                }
                else if (this.SourceEditableCollection != null)
                {
                    this.SourceEditableCollection.Add(newItem);
            ...
            // Modify our _trackingEnumerator so that it shows that our snapshot is "up to date" 
            ...
            this._internalList.Insert(addIndex, newItem);

            // fire notification for ItemCount changed
            if (!this.IsSourceCollectionPaged)
            {
                this.OnPropertyChanged("ItemCount");
            }
            ...
            this.RaiseCurrencyChanges(oldCurrentItem, oldCurrentPosition, oldIsCurrentBeforeFirst, oldIsCurrentAfterLast);
        ...
        public void CancelEdit()
        {
            ...
            if (this.SourceEditableCollection != null)
            {
                this.SourceEditableCollection.CancelEdit(editItem);
            }
            else
            {
        ...
        public void CancelNew()
        {
            ...
            int index = this.InternalIndexOf(this._newItem);
            ...
            try
            {
                ...
                if (this.SourceList != null)
                {
                    this.SourceList.Remove(this._newItem);
                }
                else if (this.SourceEditableCollection != null)
                {
                    this.SourceEditableCollection.Remove(this._newItem);
                }
            }
            finally
            {
            ...
            if (this._newItem != null)
            {
                ...
                if (addIndex > -1)
                {
                    ...
                    if (this.IsGrouping)
                    {
                        ...
                        this._group.AddToSubgroups(addItem);
                    }
                    else
                    {
        ...
        public void CommitEdit()
        {
            ...
            if (this.SourceEditableCollection != null)
            {
                this.SourceEditableCollection.EndEdit(editItem);
            }
            else
            {
                System.ComponentModel.IEditableObject ieo = editItem as System.ComponentModel.IEditableObject;
            ...
            {
                int previousIndex = this.IndexOf(editItem);

                // Apply the Filter/Sorting/Paging after the editing is complete
                try
                {
                    this._collectionChangeHandled = true;
                    this.RefreshInternal();
                }
                finally
                {
                    this._collectionChangeHandled = false;
                }

                int currentIndex = this.IndexOf(editItem);

                if (currentIndex < 0)
                {
                    // if the item was removed from the view from filtering or paging
                    this.OnCollectionChanged(
                        new NotifyCollectionChangedEventArgs(
                            NotifyCollectionChangedAction.Remove,
                            editItem,
                    ...
                    // if we are paging, we need to move in a new item
                    if (this.PageSize > 0)
                    {
                        int newItemIndex = -1;
                        if (this.InternalList.IndexOf(editItem) < this.PageSize * this.PageIndex)
                        {
                            newItemIndex = 0;
                        }
                        else
                        {
                            newItemIndex = this.Count - 1;
                        }

                        this.OnCollectionChanged(
                            new NotifyCollectionChangedEventArgs(
                                NotifyCollectionChangedAction.Add,
                                this.GetItemAt(newItemIndex),
                                newItemIndex));
                    }
                }
                else if (currentIndex != previousIndex)
                {
                    // if the item was moved due to sorting
                    this.OnCollectionChanged(
                        new NotifyCollectionChangedEventArgs(
                            NotifyCollectionChangedAction.Remove,
                            editItem,
        ...
        public void CommitNew()
        {
            ...
            if (this.UsesLocalArray)
            {
                ...
                this.ProcessInsertToCollection(newItem, this.Count);

                int pageStartIndex = (this.PageIndex - this.StartPageIndex) * this.PageSize;
                int nextPageStartIndex = pageStartIndex + this.PageSize;

                if (this.IsGrouping)
                {
                    this._temporaryGroup.AddToSubgroups(newItem);
                    int leafIndex = this._temporaryGroup.LeafIndexOf(newItem);
                    ...
                    if (this.PageSize == 0 || (pageStartIndex <= leafIndex && nextPageStartIndex > leafIndex))
                    {
                        this._group.AddToSubgroups(newItem);

                        this.OnCollectionChanged(
                            new NotifyCollectionChangedEventArgs(
                                NotifyCollectionChangedAction.Add,
                                newItem,
                                this.IndexOf(newItem)));
                    }
                    else
                    {
                        int addIndex = -1;
                        if (leafIndex < pageStartIndex)
                        {
                        ...
                        else
                        {
                            ...
                            addIndex = Math.Min(nextPageStartIndex, this._temporaryGroup.ItemCount) - 1;
                        }
        ...
        public void EditItem(object item)
        {
            ...
            {
                throw new ArgumentException(string.Format(CultureInfo.InvariantCulture, PagedCollectionViewResources.CannotEditPlaceholder), "item");
            }

            if (this.SourceEditableCollection != null && this.SourceEditableCollection.CanEdit == false)
            {
                throw new InvalidOperationException(PagedCollectionViewResources.EditItemNotSupportedByEditableCollection);
            }
            ...
            if (this.SourceEditableCollection != null)
            {
                this.SourceEditableCollection.BeginEdit(item);
            }
            else
            {
        ...
        public IEnumerator GetEnumerator()
        {
            ...
            if (this.PageSize > 0)
            {
                ...
                for (int index = this._pageSize * this.LocalPageIndex;
                    index < (int)Math.Min(this._pageSize * (this.LocalPageIndex + 1), this.InternalList.Count);
                    index++)
                {
                    list.Add(this.InternalList[index]);
                }

                return new PlaceholderAwareEnumerator(this, list.GetEnumerator(), NewItemPlaceholderPosition, this._newItem);
            }
            else
            {
                return new PlaceholderAwareEnumerator(this, this.InternalList.GetEnumerator(), NewItemPlaceholderPosition, this._newItem);
            }
        }
        ...
        public int IndexOf(object item)
        {
            ...
            {
                if ((internalIndex >= (this.LocalPageIndex * this._pageSize)) &&
                    (internalIndex < ((this.LocalPageIndex + 1) * this._pageSize)))
                {
                    return internalIndex - (this.LocalPageIndex * this._pageSize) + delta;
                }
                else
                {
                    return -1;
        ...
        public bool MoveCurrentTo(object item)
        {
            ...
            if (Object.Equals(this.CurrentItem, item) || Object.Equals(NewItemPlaceholder, item))
            {
        ...
        {
            // Check if page moves are allowed
            ...
            if (this._newItem != null || this._editItem != null)
            {
                return false;
            }
            ...
            if (!this.IsSourceCollectionPaged)
            {
                // Check if the target page is out of bound, or equal to the current page, for local paging
            ...
            if (pageIndex != -1 && this.IsSourceCollectionPaged)
            {
                Deployment.Current.Dispatcher.BeginInvoke(new RequestPageMoveDelegate(this.RequestPageMove), pageIndex);
            }
            else
            {
        ...
        public void RemoveAt(int index)
        {
            ...
            if (item == PagedCollectionView.NewItemPlaceholder)
            {
                throw new InvalidOperationException(PagedCollectionViewResources.RemovingPlaceholder);
            }
            ...
            try
            {
                ...
                if (this.SourceEditableCollection != null)
                {
                    if (this.SourceEditableCollection.CanRemove)
                    {
                        this.SourceEditableCollection.Remove(item);
                    }
                }
            ...
            int removeIndex = this.IndexOf(item);
            ...
            // trigger a reset.
            if (this.PageSize > 0 && this.Count == 0 && this.PageIndex != 0)
            {
        ...
        private static Type GetItemType(IEnumerable list)
        {
        ...
        private static object InvokePath(object item, string propertyPath)
        {
            Exception exception;
            object propertyValue = TypeHelper.GetNestedPropertyValue(item, propertyPath, out exception);
            if (exception != null)
            {
                throw exception;
            }
        ...
        /// </summary>
        private void AdjustCurrencyForNewItemPlaceholderMove()
        {
            ...
            // If CurrentItem and CurrentPosition is not in sync
            if (this.IsCurrentInView)
            {
                int previousPosition = this._currentPosition;
                this._currentPosition = this.IndexOf(this.CurrentItem);
        ...
        private void CompletePageMove(int pageIndex)
        {
            ...
            if (this._pageIndex != pageIndex)
            {
                ...
                // if we only have local paging, handle the currency update
                if (!this.IsSourceCollectionPaged)
                {
                    // update currency
                    int minItemCount = (this.NewItemPlaceholderPosition != NewItemPlaceholderPosition.None) ? 2 : 1;

                    if (this.Count >= minItemCount && this.CurrentItem != null)
                    {
                ...
                // if we only have local paging, handle the count update
                if (!this.IsSourceCollectionPaged)
                {
                    // if the count has changed
                    if (this.Count != oldCount)
        ...
        private int ConvertToInternalIndex(int index)
        {
            if (this.PageSize > 0)
            {
                return (this._pageSize * this.LocalPageIndex) + index;
            }
            else
            {
                return index;
        ...
        /// </summary>
        private void CopySnapshotToInternalList()
        {
            this._internalList = new List<object>();

            IEnumerator enumerator = this._snapshot.GetEnumerator();
        ...
        private object EndAddNew(bool cancel)
        {
            ...
            this._newItem = null;    // leave "adding-new" mode
        ...
        private void EndDefer()
        {
            ...
            {
                if (this.CheckFlag(CollectionViewFlags.IsMoveToFirstPageDeferred))
                {
                    this.SetFlag(CollectionViewFlags.IsMoveToFirstPageDeferred, false);
                    this.MoveToFirstPage();
                }
        ...
        private Type GetItemType(bool useRepresentativeItem)
        {
            ...
            {
                // collection is SomeGenericType<T> - get T
        ...
        private void LoadSnapshotCore(IEnumerable source)
        {
            ...
            else
            {
                ...
                while (this._trackingEnumerator.MoveNext())
                {
                    this._snapshot.Add(this._trackingEnumerator.Current);
                }
        ...
        /// <param name="e"></param>
        private void PagedSource_PageChanged(object sender, PageChangedEventArgs e)
        {
            this.CompletePageMove(e.PageIndex);
        }
        ...
        private void PrepareGroups()
        {
            ...
            if (this._temporaryGroup.GroupDescriptions.Count > 0)
            {
                ...
                while (num < count)
                {
                    ...
                    if (item != null && (!this.IsAddingNew || !object.Equals(this._newItem, item)))
                    {
                        this._temporaryGroup.AddToSubgroups(item);
                    }
                ...
                if (this.IsAddingNew)
                {
                    this._temporaryGroup.InsertSpecialItem(this._temporaryGroup.Items.Count, this._newItem, true);
                }
            }
            ...
            {
                this._temporaryGroup.ActiveComparer = (IComparer)Activator.CreateInstance(typeof(SortFieldComparer<>).MakeGenericType(this._elementType), this.SortDescriptions);
            }

            // this will set up the group property that faces the user with just the 
            // grouped items for the current page, or the entire collection if paging
            // is not being used.
            this.PrepareGroupsForCurrentPage();
        }
        ...
        private void PrepareGroupsForCurrentPage()
        {
            ...
            if (this.GroupDescriptions.Count > 0)
            {
                ...
                while (num < count)
                {
                    ...
                    if (item != null && (!this.IsAddingNew || !object.Equals(this._newItem, item)))
                    {
                        this._group.AddToSubgroups(item);
                    }
            ...
            if (this._elementType != null)
            {
                this._group.ActiveComparer = (IComparer)Activator.CreateInstance(typeof(SortFieldComparer<>).MakeGenericType(this._elementType), this.SortDescriptions);
            }
        ...
        private IList PrepareLocalArray<T>(IList list)
        {
            Debug.Assert(list != null, "Input list to filter/sort should not be null");
            ...
            {
                localList = SortList<T>(localList);
            }
        ...
        private void ProcessCollectionChanged(NotifyCollectionChangedEventArgs args)
        {
            ...
            if (args.Action == NotifyCollectionChangedAction.Reset)
            {
                ...
                // loading snapshot will call Refresh, which will 
            ...
            int removeIndex = (args.Action == NotifyCollectionChangedAction.Replace) ? args.NewStartingIndex : args.OldStartingIndex;
                ...
                args.Action == NotifyCollectionChangedAction.Replace)
            {
                ...
                if (this.IsGrouping)
                {
                    internalRemoveIndex = this._temporaryGroup.LeafIndexOf(removedItem);
                }
                else
                {
                ...
                bool needToRemove = this.PageSize == 0 || (internalRemoveIndex < (this.PageIndex - this.StartPageIndex + 1) * this.PageSize);
                ...
                args.Action == NotifyCollectionChangedAction.Replace)
            {
                ...
                {
                    removeNotificationItem = this.GetItemAt(this.PageSize - 1);
                }

                // process the add by filtering and sorting the item
                this.ProcessInsertToCollection(
                    addedItem,
                    (args.Action == NotifyCollectionChangedAction.Replace ? removeIndex : args.NewStartingIndex));
                ...
                if (this.IsGrouping)
                {
                    this._temporaryGroup.AddToSubgroups(addedItem);
                    int leafIndex = this._temporaryGroup.LeafIndexOf(addedItem);

                    // if we are not paging, we should just be able to add the item.
                    // otherwise, we need to validate that it is within the current page.
                    if (this.PageSize == 0 || (this.PageIndex - this.StartPageIndex + 1) * this.PageSize > leafIndex)
                    {
                        needsGrouping = true;
                ...
                if (this.PageSize > 0 && !this.OnLastLocalPage &&
                   ...
                   (!this.IsGrouping && (this.PageIndex - this.StartPageIndex + 1) * this.PageSize > this.InternalIndexOf(addedItem)))))
                {
                ...
                if (needsGrouping)
                {
                    this._group.AddToSubgroups(addedItem);
                }
                ...
                if (addedIndex >= 0)
                {
                    ...
                    this.AdjustCurrencyForAdd(addedIndex);
            ...
            if (args.Action != NotifyCollectionChangedAction.Replace && !this.IsSourceCollectionPaged)
            {
                this.OnPropertyChanged("ItemCount");
            }
        }
        ...
        private void ProcessInsertToCollection(object item, int index)
        {
            ...
            if (this.Filter == null || this.PassesFilter(item))
            {
                if (this.SortDescriptions.Count > 0)
                {
                    ...
                    if (this._elementType == null)
                    {
                        this._elementType = GetItemType(this._sourceCollection);
                    }
                    object sortFieldComparer = Activator.CreateInstance(typeof(SortFieldComparer<>).MakeGenericType(this._elementType), this.SortDescriptions);

                    MethodInfo mi = sortFieldComparer.GetType().GetMethod("FindInsertIndex");

                    // find the correct sorted index to insert into
                    index = (int)mi.Invoke(sortFieldComparer, new object[] { item, this._internalList });
                }
        ...
        private void RaiseCurrencyChanges(object oldCurrentItem, int oldCurrentPosition, bool oldIsCurrentBeforeFirst, bool oldIsCurrentAfterLast)
        {
            // fire events for currency changes
            if (this.CurrentItem != oldCurrentItem || this.CurrentPosition != oldCurrentPosition)
            {
                this.OnCurrentChanged();
            }
            if (this.CurrentItem != oldCurrentItem)
        ...
        private void RefreshOverride()
        {
            ...
            if (!this._collectionChangeHandled)
            {
                this.OnCurrentChanging();
            }
            ...
            if (this.UsesLocalArray)
            {
                if (this.InternalCount > 0 && this._snapshot.Count == 0)
                {
                    this.LoadSnapshotCore(this._sourceCollection);
                }

                // Keep track of the count before we filter/sort/group/page
                int prevCount = this.InternalCount;
                ...
                try
                {
                    ...
                    if (this._elementType != null)
                    {
                        MethodInfo mi = typeof(PagedCollectionView).GetMethod("PrepareLocalArray", BindingFlags.Instance | BindingFlags.NonPublic);
                        mi = mi.MakeGenericMethod(this._elementType);

                        // apply filtering/sorting through the PrepareLocalArray method
                        this._internalList = (IList)mi.Invoke(this, new object[] { this._snapshot });
                    }
                ...
                // if the number of items changed, fire notification
                if (!this.IsSourceCollectionPaged && prevCount != this.InternalCount)
                {
                    this.OnPropertyChanged("ItemCount");
                }
            }
            ...
            {
                this.MoveToPage(this.StartPageIndex + this.PageCount - 1);
            }
            ...
            else
            {
                ...
                if (newPosition < 0)
                {
                    ...
                    if (newPosition < this.InternalCount && (newItem = this.GetItemAt(newPosition)) != NewItemPlaceholder)
                    {
        ...
        {
            if (this.IsSourceCollectionPaged && this.PagedSource.MoveToPage(pageIndex))
            {
                return;
            }
        ...
        private void SortDescriptionsChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            ...
            if (this.PageSize > 0 && !this.IsSourceCollectionPaged)
            {
                if (this.IsRefreshDeferred)
                {
                    // set flag so that we move to first page on EndDefer
                    this.SetFlag(CollectionViewFlags.IsMoveToFirstPageDeferred, true);
                }
                else
                {
                    this.MoveToFirstPage();
        ...
        private List<T> SortList<T>(List<T> list)
        {
            ...
            foreach (SortDescription sort in this.SortDescriptions)
            {
                ...
                {
                    throw new InvalidOperationException(string.Format(CultureInfo.InvariantCulture, PagedCollectionViewResources.InvalidPropertyName, propertyPath));
                }
                ...
                {
                    throw new InvalidOperationException(PagedCollectionViewResources.FailedToCompareElements);
                }

                IOrderedEnumerable<T> orderedEnum = seq as IOrderedEnumerable<T>;
                ...
                switch (sort.Direction)
                {
                    case System.ComponentModel.ListSortDirection.Ascending:
                        ...
                        {
                            // thenby
                            seq = orderedEnum.ThenBy(item => InvokePath(item, propertyPath));
                        }
                        else
                        {
                            // orderby
                    ...
                    case System.ComponentModel.ListSortDirection.Descending:
                        ...
                        {
                            // thenby
                            seq = orderedEnum.ThenByDescending(item => InvokePath(item, propertyPath));
                        }
                        else
                        {
                            // orderby
        ...
        private class DeferHelper : IDisposable
        {
            ...
            /// </summary>
            private PagedCollectionView pagedCollectionView;
            ...
            public DeferHelper(PagedCollectionView pagedCollectionView)
            {
                this.pagedCollectionView = pagedCollectionView;
            }
            ...
            public void Dispose()
            {
                ...
                {
                    this.pagedCollectionView.EndDefer();
                    this.pagedCollectionView = null;
                }
        ...
        /// <summary>
        /// IEnumerator generated using the Placeholder and new item positions
        /// taken into account
        /// </summary>
        private class PlaceholderAwareEnumerator : IEnumerator
        {
            ...
            public PlaceholderAwareEnumerator(PagedCollectionView pagedCollectionView, IEnumerator baseEnumerator, NewItemPlaceholderPosition placeholderPosition, object newItem)
            {
                this._pagedCollectionView = pagedCollectionView;
                this._timestamp = pagedCollectionView.Timestamp;
            ...
            {
                if (this._timestamp != this._pagedCollectionView.Timestamp)
                {
            ...
            public object Current
            {
                get
                {
                    return (this._position == Position.OnPlaceholder) ? PagedCollectionView.NewItemPlaceholder
        ...
        internal class SortFieldComparer<T> : IComparer<T>, IComparer
        {
            ...
            public int Compare(T x, T y)
            {
                ...
                for (int k = 0; k < this._fields.Length; ++k)
                {
                    ...
                    result = this._fields[k].comparer.Compare(v1, v2);
            ...
            public int FindInsertIndex(T x, IEnumerable list)
            {
                int index = 0;
                IEnumerator enumerator = list.GetEnumerator();
                while (enumerator.MoveNext())
                {
                    if (this.Compare(x, enumerator.Current) == -1)
                    {
                        return index;
                    }
                    index++;
            ...
            private static SortPropertyInfo[] CreatePropertyInfo(SortDescriptionCollection sortFields)
            {
                ...
                for (int k = 0; k < sortFields.Count; ++k)
                {
                    ...
                    else
                    {
                        ...
                        if (propertyType == null)
                        {
                            throw new ArgumentException("Invalid PropertyName", sortFields[k].PropertyName);
                        }
            ...
            struct SortPropertyInfo
            {
                ...
                internal object GetValue(object o)
                {
                    ...
                    else
                    {
                        try
                        {
                            value = PagedCollectionView.InvokePath(o, this.propertyPath);
                        }
                        catch
                        {
                            value = null;
