...
namespace System.ComponentModel.DataAnnotations {
    ...
    public sealed class ValidationContext : IServiceProvider {
        ...
        /// to see if can use that instead.  Lacking that, it returns <see cref="MemberName"/>.
        /// </value>
        ...
        /// only for property or parameter contexts.  The <see cref="ObjectInstance"/> type name will be used if MemberName
        /// has not been set.</value>
        public string MemberName {
            get {
                return !string.IsNullOrEmpty(this._memberName)
                    ? this._memberName
                ...
                if (promote && this._parentContainer != null) {
                    ...
                    lock (this._lock) {
                        if (this._services.ContainsKey(serviceType)) {
                            throw new ArgumentException(string.Format(Resources.DataAnnotationsResources.ValidationContextServiceContainer_ItemAlreadyExists, serviceType), "serviceType");
                        }
