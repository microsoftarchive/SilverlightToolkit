...
namespace System.Windows.Controls.Data.Test
{
        ...
        where TDataClassSource : DataClassSource<TDataClass>, new()
    {
        ...
        public virtual void ScrollIntoView()
        {
            ...
            // unused for now
            //Action<string> AssertTopRight =
            //    delegate (string message)
            //    {
            //        Assert.IsFalse(IsCellInView(firstItem, firstColumn), message);
            //        Assert.IsTrue(IsCellInView(firstItem, lastColumn), message);
            //        Assert.IsFalse(IsCellInView(lastItem, firstColumn), message);
            //        Assert.IsFalse(IsCellInView(lastItem, lastColumn), message);
            //    };
        ...
        public void SortingDataTypes()
        {
            ...
            Action<DataGridColumn> EnqueueSortTest = (DataGridColumn col) =>
            {
                ...
                EnqueueCallback(delegate
                {
                    ...
                    // Resets happen when SortDescriptions are added and removed
                    int resetCount = dataGrid.DataConnection.SortDescriptions.Count + oldSortDescriptionCount;
                    Assert.AreEqual(dataGrid.DisplayData.NumDisplayedScrollingElements * resetCount, this._numberOfUnloadedRows);
                    Assert.AreEqual(dataGrid.DisplayData.NumDisplayedScrollingElements * resetCount, this._numberOfLoadedRows);
            ...
            Action<DataGridColumn> EnqueueSortFailTest = (DataGridColumn col) =>
            {
                EnqueueCallback(delegate
                {
                    Common.AssertExpectedException(new InvalidOperationException("Failed to compare two elements in the array."),
                        delegate
                        {
