...
namespace System.ComponentModel.DataAnnotations.Test {
    ...
    public class LocalizableStringTest {
        private const string LocalizeNotCalledExceptionMessage = "Cannot retrieve LocalizedValue because Localize(Type resourceType) has not been called.";
        ...
        public void LocalizableString_PublicStaticStringProperty_On_Internal_Class_Throws() {
            ...
            ExceptionHelper.ExpectException<InvalidOperationException>(() => { prop.GetLocalizableValue(); }, string.Format(LocalizationFailedExceptionMessage, "MyProperty", typeof(LocalizableString_Resources_NonPublic).FullName, prop.Value));
        }
        ...
        public void LocalizableString_GetLocalizedValue_Respects_Current_UI_Culture() {
            ...
            Assert.AreEqual("English", prop.GetLocalizableValue(), "GetLocalizableValue should return 'English' with the default culture");

            CultureInfo defaultCulture = CultureInfo.CurrentUICulture;
            Thread.CurrentThread.CurrentUICulture = new CultureInfo("de");

            try {
                Assert.AreEqual("German", prop.GetLocalizableValue(), "GetLocalizableValue should return 'German' with the german culture");
            } finally {
        ...
        public void LocalizableString_GetLocalizedValue_Fails_When_Resource_Key_Is_Empty() {
            ...
            prop.Value = string.Empty;
            prop.ResourceType = typeof(LocalizableString_Resources);
            ExceptionHelper.ExpectException<InvalidOperationException>(() => { prop.GetLocalizableValue(); }, string.Format(LocalizationFailedExceptionMessage, "MyProperty", prop.ResourceType.FullName, prop.Value));
        }

        private string GetExpectedErrorMessage(string propertyValue) {
            return string.Format(LocalizationFailedExceptionMessage, "MyProperty", typeof(LocalizableString_Resources).FullName, propertyValue);
        }
    }
