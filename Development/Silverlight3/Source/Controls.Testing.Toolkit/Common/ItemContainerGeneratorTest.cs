    ...
    public partial class ItemContainerGeneratorTest : TestBase
    {
        ...
        public virtual void GetContainerFromIndexOutOfRange()
        {
            ...
            TestAsync(
                ...
                () => Assert.IsNull(items.ItemContainerGenerator.ContainerFromIndex(int.MinValue), "Index int.MinValue should return null!"),
        ...
        {
            new TreeView().ItemContainerGenerator.IndexFromContainer(null);
        }
        ...
        {
            Assert.AreEqual(-1, new TreeView().ItemContainerGenerator.IndexFromContainer(new SolidColorBrush(Colors.Black)), "Expected to not find a non-UIElement DependencyObject!");
        }
        ...
        {
            new TreeView().ItemContainerGenerator.ItemFromContainer(null);
        }
        ...
        public virtual void GetItemFromContainerNotInItems()
        {
            ...
            TestAsync(
                ...
                () => Assert.IsNull(view.ItemContainerGenerator.ItemFromContainer(second), "Second should not be found!"));
        }
        ...
        public virtual void ApplyItemTemplate()
        {
            XamlBuilder<Grid> builder = new XamlBuilder<Grid>
            {
                ExplicitNamespaces = new Dictionary<string, string>
                {
                    ...
                    { "ctl", XamlBuilder.GetNamespace(typeof(ItemContainerGenerator)) },
        ...
        /// <summary>
        /// Ensure that applying a template clears the mapping.
        /// </summary>
        [TestMethod]
        [Asynchronous]
        [Description("Ensure that applying a template clears the mapping.")]
        [Bug("29340 - ItemContainerGenerator - Regression where OnApplyTemplate not called when assigning an empty template", Fixed = true)]
        public virtual void ApplyTemplateClearsMapping()
        {
            TreeView view = new TreeView { ItemsSource = new int[] { 1, 2, 3 } };
            TestAsync(
                5,
                view,
                () => Assert.IsNotNull(view.ItemContainerGenerator.ContainerFromIndex(0), "First item should not be null!"),
                () =>
                {
                    view.Template = new ControlTemplate();
                    view.ApplyTemplate();
                    Assert.IsNull(view.ItemContainerGenerator.ContainerFromIndex(0), "First item should be null!");
                });
        }
    }
}