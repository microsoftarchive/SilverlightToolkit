...
{
    internal class ListPagedCollectionView : IPagedCollectionView, INotifyCollectionChanged, INotifyPropertyChanged, IEnumerable
    {
        #region Constructors
        ...
        public ListPagedCollectionView(IList list)
        {
        ...
        public bool IsCurrentAfterLast
        {
            get
            {
                return (IsEmpty || CurrentPosition >= Count);
            }
        }
        ...
        public bool IsCurrentBeforeFirst
        {
            get
            {
                return (IsEmpty || CurrentPosition < 0);
            }
        }
        ...
        public void Refresh()
        {
            ...
            if (UsesLocalArray)
            {
                MethodInfo mi = (typeof(ListPagedCollectionView)).GetMethod("PrepareLocalArray", BindingFlags.Instance | BindingFlags.NonPublic);
                mi = mi.MakeGenericMethod(_elementType);
                try
                {
                    _internalList = (IList)mi.Invoke(this, new object[] { _internalList, _internalList != null });
                    _currentPosition = _internalList.IndexOf(_currentItem);
                }
                catch (TargetInvocationException e)
                {
                    // Refreshing failed, so we should restore the old internal current position
            ...
            {
                int index = this._internalList.IndexOf(oldCurrentItem);
                if (index < 0)
                {
                    index = 0;
                }
        ...
        /// </summary>
        /// <remarks>
        ///     PagedCollectionViews (and sub-classes) should take their filter/sort/grouping
        ...
        protected void OnCollectionChanged(object sender, NotifyCollectionChangedEventArgs args)
        {
            ...
            {
                throw PagedCollectionViewError.ListPagedCollectionView.MultiThreadedCollectionChangeNotSupported();
            }
        ...
        protected void ProcessCollectionChanged(NotifyCollectionChangedEventArgs args)
        {
            ...
            switch (args.Action)
            {
                case NotifyCollectionChangedAction.Add:
                    ...
                    if (_currentPosition <= _newItemIndex)
                    {
                        ++_currentPosition;
                    }
                    break;
                ...
                default:
                    Debug.Assert(
                        ...
                        String.Format(
                            ...
                            Resource.ListPagedCollectionView_UnexpectedCollectionChangeAction,
                            effectiveAction));
                    break;
            }
        ...
        internal static object InvokePath(object item, string propertyPath)
        {
            ...
            for (int i = 0; i < propertyNames.Length; i++)
            {
                if (newItem == null)
                {
                    throw PagedCollectionViewError.ListPagedCollectionView.InvalidPropertyName(propertyNames[i]);
                }
        ...
        private void _MoveCurrentToPosition(int position)
        {
            ...
            {
                _currentItem = this._internalList[position];
        ...
        private int AdjustBefore(NotifyCollectionChangedAction action, object item, int index)
        {
            ...
            if (index < -1 || index > ilFull.Count)
                throw PagedCollectionViewError.ListPagedCollectionView.CollectionChangeIndexOutOfRange(index, ilFull.Count);
            ...
            if (action == NotifyCollectionChangedAction.Add)
            {
                if (index >= 0)
                {
                    if (!Object.Equals(item, ilFull[index]))
                        throw PagedCollectionViewError.ListPagedCollectionView.AddedItemNotAtIndex(index);
                }
                else
                {
                    ...
                    if (index < 0)
                        throw PagedCollectionViewError.ListPagedCollectionView.AddedItemNotInCollection();
                }
            }
            ...
            if (action == NotifyCollectionChangedAction.Add)
            {
                ...
                MethodInfo mi = (typeof(ListPagedCollectionView)).GetMethod("BinarySearch", BindingFlags.Instance | BindingFlags.NonPublic);
                mi = mi.MakeGenericMethod(_elementType);
        ...
        private void AdjustCurrencyForAdd(int index)
        {
            ...
            else if (index <= _currentPosition)
            {
                ...
                {
                    _currentItem = _internalList[_currentPosition];
                }
            }
        }
        ...
        private List<T> SortList<T>(List<T> list)
        {
            ...
            foreach (System.ComponentModel.SortDescription sort in this.SortDescriptions)
            {
                ...
                {
                    throw PagedCollectionViewError.ListPagedCollectionView.InvalidPropertyName(propertyPath);
                }
                ...
                {
                    throw PagedCollectionViewError.ListPagedCollectionView.FailedToCompareElements();
                }
            ...
            switch (e.Action)
            {
                case NotifyCollectionChangedAction.Add:
                    if (e.NewItems.Count != 1)
                        throw PagedCollectionViewError.ListPagedCollectionView.RangeActionsNotSupported();
                    break;
                ...
                default:
                    throw PagedCollectionViewError.ListPagedCollectionView.UnexpectedCollectionChangeAction(e.Action);
            }
        }
