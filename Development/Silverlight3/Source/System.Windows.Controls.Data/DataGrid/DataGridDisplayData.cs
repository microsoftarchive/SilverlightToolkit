...
namespace System.Windows.Controls
{
    internal class DataGridDisplayData
    {
        ...
        internal void CorrectSlotsAfterDeletion(int slot)
        {
            if (_owner.IsSlotVisible(slot))
            {
        ...
        internal void CorrectSlotsAfterInsertion(int slot, UIElement element)
        {
            if (slot < this.FirstScrollingSlot)
            {
            ...
            else if (slot - 1 <= this.LastScrollingSlot || this.LastScrollingSlot == -1)
            {
