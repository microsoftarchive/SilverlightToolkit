...
namespace System.Windows.Controls
{
    public partial class DataGrid
    {
        ...
        public void CollapseRowGroup(CollectionViewGroup collectionViewGroup, bool collapseAllSubgroups)
        {
            ...
            EnsureRowGroupVisibility(RowGroupInfoFromCollectionViewGroup(collectionViewGroup), Visibility.Collapsed);
        ...
        public void ExpandRowGroup(CollectionViewGroup collectionViewGroup, bool expandAllSubgroups)
        {
            ...
            EnsureRowGroupVisibility(RowGroupInfoFromCollectionViewGroup(collectionViewGroup), Visibility.Visible);
        ...
        internal void InsertElementAt(int slot, int rowIndex, object item, DataGridRowGroupInfo groupInfo)
        {
            ...
            if (SlotIsDisplayed(slot))
            {
                ...
                {
                    InsertElement(slot, GenerateRowGroupHeader(slot, groupInfo), false /*updateVerticalScrollBarOnly*/);
                }
            }
            else
            {
                InsertElement(slot, null, this._vScrollBar == null || this._vScrollBar.Visibility == Visibility.Visible /*updateVerticalScrollBarOnly*/);
            }
        }
        ...
        internal void InsertRowAt(int rowIndex)
        {
            ...
            InsertElementAt(slot, rowIndex, item, null/*DataGridRowGroupInfo*/);
        }
        ...
        internal bool OnRowGroupHeaderToggled(DataGridRowGroupHeader groupHeader, Visibility newVisibility)
        {
            Debug.Assert(groupHeader.RowGroupInfo.CollectionViewGroup.ItemCount > 0);

            DataGridRowGroupHeaderToggleEventArgs e = new DataGridRowGroupHeaderToggleEventArgs(groupHeader);
            if (newVisibility == Visibility.Visible)
            {
                OnRowGroupExpanding(e);
            }
            else
            {
                OnRowGroupCollapsing(e);
            }
            if (e.Cancel || !this.CommitEdit())
            {
                return false;
            }

            if (this.CurrentSlot != groupHeader.RowGroupInfo.Slot)
            {
        ...
        private void ClearRows(bool recycle)
        {
            ...
            // If we're clearing the rows, there shouldn't be an editing row anymore and
            // we should also make sure to clear the focused row (because it's no longer relevant)
        ...
        private double CollapseSlotsInTable(int startSlot, int endSlot, ref int slotsExpanded)
        {
            ...
            while (firstSlot <= endSlot)
            {
                ...
                {
                    double headerHeight;
                    double rowCount = lastSlot - firstSlot - GetRowGroupHeaderCount(firstSlot, lastSlot, Visibility.Visible, out headerHeight) + 1;
                    double detailsCount = GetDetailsCountInclusive(firstSlot, lastSlot);

                    heightChange -= headerHeight + (detailsCount * this.RowDetailsHeightEstimate) + (rowCount * this.RowHeightEstimate);
        ...
        private void CorrectRowAfterDeletion(DataGridRow row, bool rowDeleted)
        {
            row.Slot--;
            if (rowDeleted)
            {
                row.Index--;
                if (IsSlotVisible(row.Slot) && this.RowGroupHeadersTable.IndexCount > 0)
                {
                    // There is an edge case where deleting a row causes multiple
                    // RowGroupHeaders to disappear at the same time and then we end
                    // up temporarily generating a row with the wrong index when
                    // we're populating the tree
                    row.DataContext = this.DataConnection.GetDataItem(row.Index);
                }
            }
        }
        ...
        /// </summary>
        private void CorrectSlotsAfterInsertion(int slotInserted)
        {
            ...
            // Take care of the non-visible edited row
            if (this.EditingRow != null &&
                !this.IsSlotVisible(EditingRow.Slot) &&
                this.EditingRow.Slot >= slotInserted)
            {
                this.EditingRow.Index++;
                this.EditingRow.Slot++;
            }
            ...
            // Update the RowGroupHeaders
            foreach (int slot in this.RowGroupHeadersTable.GetIndexes(this.RowGroupHeadersTable.GetPreviousIndex(slotInserted)))
            {
                DataGridRowGroupInfo rowGroupInfo = this.RowGroupHeadersTable.GetValueAt(slot);
            ...
            switch (e.Action)
            {
                case NotifyCollectionChangedAction.Add:
                    if (e.NewItems != null && e.NewItems.Count > 0)
                    {
                        ...
                        int insertSlot = -1;
                        DataGridRowGroupInfo parentGroupInfo = null;
                        CollectionViewGroup group = e.NewItems[0] as CollectionViewGroup;

                        if (sender == this.DataConnection.CollectionView.Groups)
                        {
                            // If the new item is a root level element, it has no parent group, so create an empty RowGroupInfo
                            parentGroupInfo = new DataGridRowGroupInfo(null, Visibility.Collapsed, -1, -1, -1);
                        }
                        else
                        {
                            foreach (int slot in this.RowGroupHeadersTable.GetIndexes())
                            {
                                parentGroupInfo = this.RowGroupHeadersTable.GetValueAt(slot);
                                if (parentGroupInfo.CollectionViewGroup.Items == sender)
                                {
                                    break;
                                }
                                parentGroupInfo = null;
                            }
                        }
                        ...
                        if (insertSlot != -1)
                        {
                            if (group != null)
                            {
                                ...
                                InsertElementAt(insertSlot, -1 /*rowIndex*/, null /*item*/, newGroupInfo);
                            ...
                            else
                            {
                                ...
                                Debug.Assert(rowIndex != -1);
                                InsertElementAt(insertSlot, rowIndex, e.NewItems[0]/*item*/, null/*rowGroupInfo*/);
                            }
        ...
        private void EnsureAnscestorsExpanderButtonChecked(DataGridRowGroupInfo parentGroupInfo)
        {
            if (IsSlotVisible(parentGroupInfo.Slot))
            {
                ...
                {
                    ancestorGroupHeader.EnsureExpanderButtonChecked();
        ...
        private void EnsureRowGroupVisibility(DataGridRowGroupInfo rowGroupInfo, Visibility visibility)
        {
            ...
            if (rowGroupInfo.Visibility != visibility)
            {
                if (this.IsSlotVisible(rowGroupInfo.Slot))
                {
                    ...
                    rowGroupHeader.ToggleExpandCollapse(true /*updateExpanderButton*/);
                }
                else
                {
        ...
        private void ExpandSlots(int startSlot, int endSlot, bool isDisplayed, ref int slotsExpanded, ref double heightChange)
        {
            ...
            if (isDisplayed)
            {
                ...
                for (int i = startSlot; (i <= endSlot) && (heightChange < availableHeight); i++)
                {
                    FrameworkElement insertedElement = InsertDisplayedElement(i, false /*updateSlotInformation*/);
                    heightChange += insertedElement.DesiredSize.Height;
        ...
        private DataGridRow GetGeneratedRow(int rowIndex)
        {
            ...
            if (this.EditingRow != null && rowIndex == this.EditingRow.Index)
            {
        ...
        private void InsertDisplayedElement(int slot, UIElement element, bool wasNewlyAdded, bool updateSlotInformation)
        {
            ...
            if (wasNewlyAdded)
            {
                this.DisplayData.CorrectSlotsAfterInsertion(slot, element);
            }
            else
            {
        ...
        private void InsertElement(int slot, UIElement element, bool updateVerticalScrollBarOnly)
        {
            Debug.Assert(slot >= 0 && slot <= this.SlotCount);

            DataGridCellCoordinates newCurrentCellCoordinates = new DataGridCellCoordinates(-1, -1);

            OnInsertingElement(slot, ref newCurrentCellCoordinates, true /*firstInsertion*/);   // will throw an exception if the insertion is illegal

            OnInsertedElement_Phase1(slot, element);
            this.SlotCount++;
            this.VisibleSlotCount++;
            OnInsertedElement_Phase2(slot, newCurrentCellCoordinates, true /*lastInsertion*/, updateVerticalScrollBarOnly);
            OnElementsChanged(true /*grew*/);
        }
        ...
        private void OnInsertedElement_Phase1(int slot, UIElement element)
        {
            Debug.Assert(slot >= 0);

            // Fix the Index of all following rows
            CorrectSlotsAfterInsertion(slot);
            ...
            else if (slot <= this.DisplayData.FirstScrollingSlot)
            {
                this.DisplayData.CorrectSlotsAfterInsertion(slot, null /*row*/);
            }
        }

        private void OnInsertedElement_Phase2(int slot, DataGridCellCoordinates newCurrentCellCoordinates,
                                          bool lastInsertion, bool updateVerticalScrollBarOnly)
        {
            ...
            if (lastInsertion && newCurrentCellCoordinates.Slot != -1)
            {
                ...
                SetAndSelectCurrentCell(newCurrentCellCoordinates.ColumnIndex,
                                        ...
                                        this.SlotCount == 1 /*forceCurrentCellSelection*/);
            }
        }
                                    ...
                                    bool firstInsertion)
        {
            ...
            if (firstInsertion)
            {
                ...
                {
                    newCurrentCellCoordinates = new DataGridCellCoordinates(this.CurrentColumnIndex, this.CurrentSlot + 1);
            ...
            {
                if (newCurrentCellCoordinates.Slot != -1)
                {
                    newCurrentCellCoordinates.Slot++;
                }
            }
        ...
        private void OnRemovedElement(int slotDeleted, bool wasRow, DataGridCellCoordinates newCurrentCellCoordinates, bool forceCurrentCellSelection)
        {
            CorrectSlotsAfterDeletion(slotDeleted, wasRow);
            ...
            {
                if (slotDeleted >= this.DisplayData.FirstScrollingSlot)
                {
                    // Displayed row is removed
                    deletedElement = RemoveDisplayedElement(slotDeleted, true /*wasDeleted*/, true /*updateSlotInformation*/);
                }
                else
                {
                    // Removed row is above our viewport, just update the DisplayData
                    this.DisplayData.CorrectSlotsAfterDeletion(slotDeleted);
                }
            }
            ...
            {
                Debug.Assert(this.CurrentColumnIndex == -1);
                SetAndSelectCurrentCell(newCurrentCellCoordinates.ColumnIndex, newCurrentCellCoordinates.Slot, forceCurrentCellSelection);
            }
            // Raise SelectionChange event if needed
            FlushSelectionChanged();

            if (slotDeleted >= this.DisplayData.LastScrollingSlot && deletedElement == null)
            {
            ...
            else
            {
                if (deletedElement != null)
                {
                    ...
                    this.AvailableSlotElementRoom += deletedElement.DesiredSize.Height;
                }
                else
                {
        ...
        private DataGridCellCoordinates OnRemovingElement(int slotDeleted, object itemDeleted)
        {
            ...
            {
                int newCurrentSlot;
                if (slotDeleted == this.CurrentSlot && this.CurrentSlot != this.LastVisibleSlot)
                {
                    newCurrentSlot = GetNextVisibleSlot(this.CurrentSlot - 1);
                }
                else
                {
                    newCurrentSlot = GetPreviousVisibleSlot(this.CurrentSlot);
                }
                newCurrentCellCoordinates = new DataGridCellCoordinates(this.CurrentColumnIndex, newCurrentSlot);
        ...
        private void RefreshRowGroupHeaders()
        {
            ...
            if (this.DataConnection.CollectionView != null && this.DataConnection.CollectionView.Groups != null)
            {
                ...
                ((INotifyCollectionChanged)this.DataConnection.CollectionView.Groups).CollectionChanged -= CollectionViewGroup_CollectionChanged;
            }
        ...
        private DataGridRow RemoveDisplayedElement(int slot, bool wasDeleted, bool updateSlotInformation)
        {
            Debug.Assert(slot >= this.DisplayData.FirstScrollingSlot &&
                         slot <= this.DisplayData.LastScrollingSlot);

            UIElement element = this.DisplayData.GetDisplayedElement(slot);
            DataGridRow dataGridRow = element as DataGridRow;
            if (dataGridRow != null)
            {
            ...
            if (wasDeleted)
            {
                this.DisplayData.CorrectSlotsAfterDeletion(slot);
            }
            else
            {
        ...
        private void RemoveElementAt(int slot, object item, bool isRow)
        {
            ...
            DataGridCellCoordinates newCurrentCellCoordinates = OnRemovingElement(slot, item);
            this.SlotCount--;
            this.VisibleSlotCount--;
            OnRemovedElement(slot, isRow, newCurrentCellCoordinates, this.SelectedItem == item /*forceCurrentCellSelection*/);
            OnElementsChanged(false /*grew*/);
        }
        ...
        private bool SlotIsDisplayed(int slot)
        {
            ...
            else if (slot == this.DisplayData.LastScrollingSlot + 1)
            {
        ...
        private double UpdateRowGroupVisibility(DataGridRowGroupInfo targetRowGroupInfo, Visibility newVisibility, bool isDisplayed)
        {
            ...
            else
            {
                ...
                if (this.DisplayData.FirstScrollingSlot >= startSlot && this.DisplayData.FirstScrollingSlot <= endSlot)
                {
                    ...
                    heightChange += CollapseSlotsInTable(startSlot, endSlot, ref slotsExpanded);
                    if (isDisplayed)
                    {
                ...
                if (this.DisplayData.LastScrollingSlot >= startSlot && this.DisplayData.LastScrollingSlot <= endSlot)
                {
                    ...
                    int oldLastScrollingSlot = this.DisplayData.LastScrollingSlot;
                    this.DisplayData.LastScrollingSlot = GetPreviousVisibleSlot(this.DisplayData.LastScrollingSlot);

                    if (GetNextVisibleSlot(oldLastScrollingSlot) >= this.VisibleSlotCount + slotsExpanded)
                    {
                        // Collapsed the bottom most rows, adjust the vertical offset
                        int bottomSlot = this.DisplayData.LastScrollingSlot + 1;
                        int detailsCount = GetDetailsCountInclusive(bottomSlot, oldLastScrollingSlot);
                        double groupHeaderHeight;
                        int groupHeaderCount = GetRowGroupHeaderCount(bottomSlot, oldLastScrollingSlot, null/*visibility*/, out groupHeaderHeight);
                        int rowCount = oldLastScrollingSlot - this.DisplayData.LastScrollingSlot - groupHeaderCount;
                        SetVerticalOffset(Math.Max(0, _verticalOffset - (this.RowHeightEstimate * rowCount + detailsCount * this.RowDetailsHeightEstimate + groupHeaderHeight)));
                    }
                }
            }
