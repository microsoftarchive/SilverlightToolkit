    ...
    public partial class DataGrid
    {
        ...
        private void AutoGenerateColumnsPrivate()
        {
            ...
            try
            {
                ...
                {
                    object[] attributes = this.DataConnection.DataType.GetCustomAttributes(typeof(BindableAttribute), false);

                    // If BindableAttribute is defined on the class
                    if (attributes != null && attributes.Length > 0)
                    {
                        BindableAttribute classAttr = attributes[0] as BindableAttribute;
                        Debug.Assert(classAttr != null);

                        if (classAttr.Bindable)
                        {
                            GenerateColumnsFromProperties(classAttr.Direction);
                        }
                    }
                    else
                    {
                        // if the class does not have any attribute , we are passing in BindingDirection.TwoWay since it has no effect
                        GenerateColumnsFromProperties(BindingDirection.TwoWay);
                    }
                }
                EnsureRowsPresenterVisibility();
                InvalidateRowHeightEstimate();
        ...
        private bool GenerateColumn(Type propertyType, string propertyName, string header, BindingDirection direction)
        {
            ...
            if (String.IsNullOrEmpty(propertyName) || direction == BindingDirection.OneWay)
            {
            ...
            DataGridAutoGeneratingColumnEventArgs e = new DataGridAutoGeneratingColumnEventArgs(propertyName, propertyType, newColumn);
        ...
        private void GenerateColumnsFromProperties(BindingDirection direction)
        {
            ...
            if (this.DataConnection.DataProperties != null && this.DataConnection.DataProperties.Length > 0)
            {
                ...
                {
                    object[] attributes = propertyInfo.GetCustomAttributes(typeof(BindableAttribute), false);

                    // If BindableAttribute is defined on the property
                    if (attributes != null && attributes.Length > 0)
                    {
                        BindableAttribute propAttr = attributes[0] as BindableAttribute;
                        Debug.Assert(propAttr != null);

                        if (propAttr.Bindable)
                        {
                            GenerateColumn(propertyInfo.PropertyType, propertyInfo.Name, propertyInfo.Name, propAttr.Direction);
                        }
                    }
                    else
                    {
                        GenerateColumn(propertyInfo.PropertyType, propertyInfo.Name, propertyInfo.Name, direction);
                    }
                }
            }
            else if (this.DataConnection.DataIsPrimitive)
            {
                GenerateColumn(this.DataConnection.DataType, string.Empty, "value", direction);
            }
        }
