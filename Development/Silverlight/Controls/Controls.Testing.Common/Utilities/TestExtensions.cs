...
namespace Microsoft.Windows.Controls.Testing
{
    ...
    public static partial class TestExtensions
    {
        ...
        {
            foreach (DependencyObject obj in parent.GetVisualChildren())
            {
                if (obj == null)
                {
                    continue;
        ...
        /// <summary>
        /// This helper method is designed to find the first child element that has a Type of type, using VisualTreeHelper.
        /// </summary>
        /// <param name="root">Root is the UI element root.</param>
        /// <param name="type">Type is the specified type that the method needs to find from the root's children and return.</param>
        /// <returns>The child element that is type of "type".</returns>
        public static FrameworkElement GetChildrenByType(FrameworkElement root, Type type)
        {
            if (root == null)
            {
                return null;
