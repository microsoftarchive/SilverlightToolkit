...
namespace System.Windows.Controls
{
    ...
    internal sealed partial class AccordionItemContainerGenerator : IItemContainerGenerator
    {
        /// <summary>
        ...
        /// </param>
        internal AccordionItemContainerGenerator(ItemsControl control)
        {
        ...
        internal void PrepareContainerForItemOverride(DependencyObject element, object item, Style parentItemContainerStyle)
        {
            ...
            {
                PrepareHeaderedContentControlContainer(headeredContentControl, item, ItemsControl);
            }
        }

        /// <summary>
        /// Prepares the headered content control container.
        /// </summary>
        /// <param name="control">The control that will display the item.</param>
        /// <param name="item">The item that needs to be prepared.</param>
        /// <param name="parentItemsControl">The parent items control.</param>
        [SuppressMessage("Microsoft.Usage", "CA1801:ReviewUnusedParameters", MessageId = "parentItemsControl", Justification = "Matching ItemsControl signature.")]
        private static void PrepareHeaderedContentControlContainer(HeaderedContentControl control, object item, ItemsControl parentItemsControl)
        {
            if (control != item)
            {
                // We do not have proper access to Visual.
                // Nor do we keep track of the HeaderIsItem property.
                if (!(item is UIElement) && HasDefaultValue(control, HeaderedContentControl.HeaderProperty))
                {
                    control.Header = item;
                }
            }
        ...
        [SuppressMessage("Microsoft.Performance", "CA1811:AvoidUncalledPrivateCode", Justification = "Follows ItemContainerGenerator functionality.")]
