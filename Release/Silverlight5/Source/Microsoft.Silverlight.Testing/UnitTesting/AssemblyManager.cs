...
namespace Microsoft.Silverlight.Testing.Harness
{
    ...
    public class AssemblyManager : UnitTestCompositeWorkItem
    {
        ...
        protected override void FirstInvoke()
        {
            ...
            TestHarness.Events.SendTestAssemblyStarting(new TestAssemblyStartingEventArgs(_assembly, TestHarness));

            EnqueueAssemblyInitialize();
            EnqueueTestClasses();
            EnqueueAssemblyCleanup();

            Complete += (object sender, EventArgs e) =>
                {
                    LogWriter.GranularAssemblyTestStage(_assembly, TestGranularity.TestGroup, TestStage.Finishing);
                    TestHarness.Events.SendTestAssemblyCompleted(new TestAssemblyCompletedEventArgs(_assembly, TestHarness));
                };
        }
