...
namespace Microsoft.Silverlight.Testing.Harness
{
    ...
    public class TestClassManager : UnitTestCompositeWorkItem
    {
        ...
        protected override void FirstInvoke()
        {
            ...
            EnqueueQuick(() => 
                {
                    LogWriter.TestClassStage(_testClass, TestStage.Starting);
                    TestHarness.Events.SendTestClassStarting(new TestClassStartingEventArgs(_testClass, TestHarness));
                });
            ...
            EnqueueQuick(() =>
                {
                    LogWriter.TestClassStage(_testClass, TestStage.Finishing);
                    TestHarness.Events.SendTestClassCompleted(new TestClassCompletedEventArgs(_testClass, TestHarness));
                });
        }
