...
{
    [CustomValidation(typeof(CustomerValidator), "IsFullNameValidLength")]
    [CustomValidation(typeof(CustomerValidator), "IsRatingValid")]
    [CustomValidation(typeof(CustomerValidator), "IsRegisteredValid")]
    public class Customer : INotifyPropertyChanged, IEditableObject
    {
        ...
        public Customer()
        {
            ...
            this._data._firstName = GetFakeString(_random);
            this._data._lastName = GetFakeString(_random);
            this._data._preferredColor = GetFakeColor(_random);
            this._data._rating = _random.Next(5);
            this._data._fullAddress = GetFakeString(_random);
            this._data._yearlyFees = (decimal)(_random.Next(100, 10000) / 100.0);
            this._data._isRegistered = GetFakeBoolean(_random);
            this._data._isValid = GetFakeNullableBoolean(_random);
            this._data._renewalDate = DateTime.Now.AddDays(-_random.Next(364));
        }

        public Customer(string firstName, string lastName, Color preferredColor, int rating, DateTime renewalDate, string fullAddress, decimal yearlyFees, bool isRegistered, bool? isValid)
            : this()
        {
            this._delayPropertyChangeNotifications = true;
            this._data = new CustomerData();
    ...
    public class CustomerList : IList<Customer>, IList, INotifyCollectionChanged
    {
        ...
        public CustomerList()
        {
            ...
            Add(new Customer("Maurizio", "----i", Colors.Yellow, 1, DateTime.Now, "Strada Provinciale 124, 42100 Reggio Emilia, Italy", (decimal)11.5, true, false));
    ...
    {
        #region IsFullNameValidLength

        public static ValidationResult IsFullNameValidLength_ValidationResult =
            new ValidationResult("FirstName and LastName cannot exceed 25 characters combined!",
                new List<string>() { "FirstName", "LastName" });

        public static ValidationResult IsFullNameValidLength(object customerObject, ValidationContext context)
        {
            ...
            bool isValid = (customer.FirstName.Length + customer.LastName.Length) <= 25;
            if (!isValid)
            {
                return IsFullNameValidLength_ValidationResult;
            }
            return ValidationResult.Success;
        }

        #endregion IsFullNameValidLength

        #region IsRegisteredValid

        public static ValidationResult IsRegisteredValid_ValidationResult =
            new ValidationResult("IsValid cannot be true if IsRegistered is false!",
                new List<string>() { "IsValid", "IsRegistered" });

        public static ValidationResult IsRegisteredValid(object customerObject, ValidationContext context)
        {
            Customer customer = customerObject as Customer;
            if (!customer.Validate)
            {
            ...
            bool isValid = !(customer.IsRegistered == false && customer.IsValid == true);
            if (!isValid)
            {
                return IsRegisteredValid_ValidationResult;
            }
            return ValidationResult.Success;
        }

        #endregion IsRegisteredValid

        #region IsRatingValid

        public static ValidationResult IsRatingValid_ValidationResult =
            new ValidationResult("Rating cannot be negative!", new List<string>() { "Customer" });

        public static ValidationResult IsRatingValid(Customer customer, ValidationContext context)
        {
            // Normally you wouldn't have a validation rule like this because you would:
            // (a) do this kind of validation on the property itself, and
            // (b) specify which member name is involved.
            // We have this rule to test the case that no member name is specified in the ValidationResult.
            if (!customer.Validate)
            {
                return ValidationResult.Success;
            }
            bool isValid = customer.Rating >= 0;
            if (!isValid)
            {
                return IsRatingValid_ValidationResult;
            }
            return ValidationResult.Success;
        }

        #endregion IsRatingValid
    }
}
