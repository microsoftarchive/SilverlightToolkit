...
namespace System.Windows.Navigation.UnitTests
{
    ...
    public class PageResourceContentLoaderTests : SilverlightTest
    {
        ...
        private INavigationContentLoader PageResourceContentLoader { get; set; }
        ...
        public void NullUriIsError()
        {
            ...
            this.EnqueueCallback(() => this.PageResourceContentLoader.BeginLoad(null,null,callback,null));
        ...
        public void LoadPureXaml()
        {
            ...
            AsyncCallback callback =
                (asyncResult) =>
                {
                    Assert.IsFalse(asyncResult.CompletedSynchronously);
                    Assert.IsTrue(asyncResult.IsCompleted);
                    LoadResult loadResult = this.PageResourceContentLoader.EndLoad(asyncResult);
                    Assert.IsInstanceOfType(loadResult.LoadedContent, typeof(Page));
                    complete = true;
                };

            this.EnqueueCallback(() => this.PageResourceContentLoader.BeginLoad(uriToPureXaml, null, callback, null));
        ...
        public void LoadXamlWithXClass()
        {
            ...
            AsyncCallback callback =
                (asyncResult) =>
                {
                    ...
                    Assert.IsTrue(asyncResult.IsCompleted);
                    LoadResult loadResult = this.PageResourceContentLoader.EndLoad(asyncResult);
                    Assert.IsInstanceOfType(loadResult.LoadedContent, typeof(PageWithCodebehind));

                    complete = true;
                };

            this.EnqueueCallback(() => this.PageResourceContentLoader.BeginLoad(uriToXamlWithCodebehind, null, callback, null));
            this.EnqueueConditional(() => complete);
            this.EnqueueTestComplete();
        }
        ...
        public void LoadXamlWithXClassAndComment()
        {
            ...
            AsyncCallback callback =
                (asyncResult) =>
                {
                    ...
                    Assert.IsTrue(asyncResult.IsCompleted);
                    LoadResult loadResult = this.PageResourceContentLoader.EndLoad(asyncResult);
                    Assert.IsInstanceOfType(loadResult.LoadedContent, typeof(PageWithCodebehindAndComment));

                    complete = true;
                };

            this.EnqueueCallback(() => this.PageResourceContentLoader.BeginLoad(uriToXamlWithCodebehindAndComment, null, callback, null));
            this.EnqueueConditional(() => complete);
            this.EnqueueTestComplete();
        }
