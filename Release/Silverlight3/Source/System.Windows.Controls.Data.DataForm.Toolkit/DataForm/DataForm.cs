...
namespace System.Windows.Controls
{
    ...
    public class DataForm : Control
    {
        ...
        private static void OnEditTemplatePropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (dataForm.CurrentItemType != null && dataForm._currentlyUsedTemplate.ContainsKey(dataForm.CurrentItemType) && dataForm.ShouldRegenerateUI(DataFormMode.Edit))
            {
                if (dataForm.Mode == DataFormMode.ReadOnly)
                {
                    dataForm.RemoveContentForType(dataForm.CurrentItemType, dataForm._readOnlyContentsNew);
                    dataForm.RemoveContentForType(dataForm.CurrentItemType, dataForm._readOnlyContentsOld);
                }
                else if (dataForm.Mode == DataFormMode.Edit)
                {
                    dataForm.RemoveContentForType(dataForm.CurrentItemType, dataForm._editContentsNew);
                    dataForm.RemoveContentForType(dataForm.CurrentItemType, dataForm._editContentsOld);
                }
                else
                {
                    dataForm.RemoveContentForType(dataForm.CurrentItemType, dataForm._addNewContentsNew);
                    dataForm.RemoveContentForType(dataForm.CurrentItemType, dataForm._addNewContentsOld);
                }

                dataForm.GenerateUI(false /* clearEntityErrors */);
            }
        ...
        private static void OnNewItemTemplatePropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (dataForm.CurrentItemType != null && dataForm._currentlyUsedTemplate.ContainsKey(dataForm.CurrentItemType) && dataForm.ShouldRegenerateUI(DataFormMode.AddNew))
            {
                if (dataForm.Mode == DataFormMode.ReadOnly)
                {
                    dataForm.RemoveContentForType(dataForm.CurrentItemType, dataForm._readOnlyContentsNew);
        ...
        private static void OnReadOnlyTemplatePropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (dataForm.CurrentItemType != null && dataForm._currentlyUsedTemplate.ContainsKey(dataForm.CurrentItemType) && dataForm.ShouldRegenerateUI(DataFormMode.ReadOnly))
            {
                if (dataForm.Mode == DataFormMode.ReadOnly)
                {
                    dataForm.RemoveContentForType(dataForm.CurrentItemType, dataForm._readOnlyContentsNew);
        ...
        /// </summary>
        private IList<Exception> _fieldLevelErrors;
        ...
        public DataForm()
        {
            ...
            this._fieldLevelErrors = new List<Exception>();
        ...
        /// </summary>
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Maintainability", "CA1502:AvoidExcessiveComplexity", Justification = "No need to split up the method; it only has a large number of if statements.")]
        ...
        private bool BeginEdit(bool startingNewEdit)
        {
            ...
            if (e.Cancel)
            {
                ...
                {
                    throw new InvalidOperationException(string.Format(Globalization.CultureInfo.InvariantCulture, System.Windows.Controls.Data.DataForm.Toolkit.Resources.DataForm_CannotCancelBeginEditWhenInAutoEdit, "BeginEdit", "AutoEdit"));
                }
            }
        ...
        private void RemoveContentForType(Type type, IDictionary<Type, FrameworkElement> contents)
        {
            ...
            {
                IList<DataField> fields = this._fieldsDictionary[content];
        ...
        private void OnContentRootElementBindingValidationError(object sender, ValidationErrorEventArgs e)
        {
            switch (e.Action)
            {
                case ValidationErrorEventAction.Added:
                    if (!this._fieldLevelErrors.Contains(e.Error.Exception))
                    {
                        this._fieldLevelErrors.Add(e.Error.Exception);
                    }
                    break;
                ...
                case ValidationErrorEventAction.Removed:
                    if (this._fieldLevelErrors.Contains(e.Error.Exception))
                    {
                        this._fieldLevelErrors.Remove(e.Error.Exception);
                    }
                    break;
            }
