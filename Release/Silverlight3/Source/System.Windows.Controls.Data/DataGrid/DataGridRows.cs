...
namespace System.Windows.Controls
{
    public partial class DataGrid
    {
        ...
        private void InsertElement(int slot, UIElement element, bool updateVerticalScrollBarOnly, bool isCollapsed)
        {
            Debug.Assert(slot >= 0 && slot <= this.SlotCount);

            OnInsertingElement(slot, true /*firstInsertion*/, isCollapsed);   // will throw an exception if the insertion is illegal

            OnInsertedElement_Phase1(slot, element, isCollapsed);
            this.SlotCount++;
            if (!isCollapsed)
            {
                this.VisibleSlotCount++;
        ...
        private void OnInsertedElement_Phase1(int slot, UIElement element, bool isCollapsed)
        {
            Debug.Assert(slot >= 0);

            // Fix the Index of all following rows
            DataGridRow dataGridRow = element as DataGridRow;
            CorrectSlotsAfterInsertion(slot, isCollapsed, dataGridRow != null);
