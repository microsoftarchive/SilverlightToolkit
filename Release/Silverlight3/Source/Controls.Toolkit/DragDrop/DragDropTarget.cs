...
namespace System.Windows.Controls
{
...
#if SILVERLIGHT
    public abstract class DragDropTarget<TItemsControlType, TItemContainerType> : ContentControl, IDisposable, SW.IAcceptDrop
#else
    public abstract class DragDropTarget<TItemsControlType, TItemContainerType> : ContentControl, IDisposable
#endif
        ...
        where TItemContainerType : FrameworkElement
    {
                ...
                if (dropTarget != null)
                {
                    try
                    {
                        ...
                        if ((_lastGiveFeedbackEffects & SW.DragDropEffects.Move) == SW.DragDropEffects.Move)
                        {
                            ...
                            RemoveDataFromItemsControl((TItemsControlType)_currentItemDragEventArgs.DragSource, selectionCollection);
                        }
                        OnDropOverride(args);
                    }
                    finally
        ...
        protected virtual void OnIndicatingInsertionLocation(IndicatingInsertionLocationEventArgs<TItemsControlType> args)
        {
            ...
            if (_insertionIndicatorContainer != null && _insertionIndicator != null)
            {
                ...
                if (element != null)
                {
                    ...
                    {
                        _insertionIndicatorContainer.Clip = new RectangleGeometry { Rect = new Rect(origin, element.RenderSize) };
                    }
                }
        ...
        /// </summary>
        public event SW.DragEventHandler DragEnter
        {
            add
            {
        ...
        /// </summary>
        public event SW.DragEventHandler DragOver
        {
            add
            {
                _dragOver.Add(value);
        ...
        /// </summary>
        public event SW.DragEventHandler DragLeave
        {
            add
            {
        ...
        /// An event raised when the an item is dropped onto the adorner.
        /// </summary>
        public event SW.DragEventHandler Drop
        {
            add
            {
                _drop.Add(value);
        ...
        protected DragDropTarget()
        {
            ...
            IObservable<Event<MouseEventArgs>> draggingObservable =
                from dragStarted in _itemDragStarted
                from mouseMove in
                    Application.Current.RootVisual
                        .GetMouseMoveOnSelfAndSiblings()
                        .Until(_itemDragCompleted)
                select mouseMove;
        ...
        private IObservable<SW.DragDropKeyStates> GetKeyStatesChanged()
        {
            return
                ...
                // before drag starts listen only root visual
                    ...
                    from keyStates in
                        Application.Current.RootVisual
                            .GetKeyStateChangedAlways(_keyStates)
                            .Until(_itemDragStarted)
                    select keyStates,
                // after drag starts listen to root visual and all siblings
                    from _ in _itemDragStarted
                    from keyStates in
                        Application.Current.RootVisual
                            .GetKeyStateChangedOnSelfAndSiblingsAlways(_keyStates)
                            .Until(_itemDragCompleted)
                    select keyStates);
        }
            ...
            return
                ...
                from dragStarted in
                    Application.Current.RootVisual
                        ...
                        .Take(1)
                        .Until(Application.Current.RootVisual.GetMouseLeftButtonUpOnSelfAndSiblingsAlways())
        ...
        /// <summary>
        /// Removes data from the ItemsControl.
        /// </summary>
        /// <param name="itemsControl">The items control.</param>
                ...
                if (itemContainer != null)
                {
                    ...
                    if (orientation != null)
                    {
                        ...
                        if (gt != null)
                        {
                            ...
                            {
                                Point offset = gt.Transform(new Point(0, insertBefore ? 0 : itemContainer.RenderSize.Height));

                                if (offset.Y <= _insertionIndicatorContainer.RenderSize.Height)
                                {
                                    return
                                        new LineGeometry
                                        {
                                            StartPoint = offset,
                                            EndPoint = new Point(offset.X + itemContainer.RenderSize.Width, offset.Y)
                                        };
                                }
                            }
                            else
                            {
                                Point offset = gt.Transform(new Point(insertBefore ? 0 : itemContainer.RenderSize.Width, 0));

                                if (offset.X <= _insertionIndicatorContainer.RenderSize.Height)
                                {
                                    return
                                        new LineGeometry
                                        {
                                            StartPoint = offset,
                                            EndPoint = new Point(offset.X, offset.Y + itemContainer.RenderSize.Height)
                                        };
                                }
                            }
                        }
                    ...
                    if (orientation != null)
                    {
                        ...
                        insertBefore =
                                (orientation == Orientation.Horizontal
                                    && relativePoint.X < targetItemContainer.RenderSize.Width / 2.0)
                                || (orientation == Orientation.Vertical
                                    && relativePoint.Y < targetItemContainer.RenderSize.Height / 2.0);
        ...
        {
            _itemDragCompleted.Dispose();
            _itemDragStarted.Dispose();
            _itemsControlChanged.Dispose();
            _itemDragStarted.Dispose();
        }
    }
}