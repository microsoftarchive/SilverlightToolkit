...
namespace System.Windows.Controls
{
    ...
    public class TreeViewDragDropTarget : ItemsControlDragDropTarget<ItemsControl, TreeViewItem>
    {
        ...
        /// </summary>
        private ISubject<Event<SW.DragEventArgs>> _dragEnter = new Subject<Event<SW.DragEventArgs>>();

        /// <summary>
        /// An event raised when a drag leave operation occurs.
        /// </summary>
        private ISubject<Event<SW.DragEventArgs>> _dragLeave = new Subject<Event<SW.DragEventArgs>>();

        /// <summary>
        /// An event raised when a drop operation occurs.
        /// </summary>
        private ISubject<Event<SW.DragEventArgs>> _drop = new Subject<Event<SW.DragEventArgs>>();
        ...
        protected override void OnContentChanged(object oldContent, object newContent)
        {
            ...
            {
                throw new ArgumentException(System.Windows.Controls.Properties.Resources.TreeViewDragDropTarget_set_Content_ContentMustBeATreeView);
            }
        ...
        public TreeViewDragDropTarget()
        {
            ...
            SynchronizationContext uiContext = System.Threading.SynchronizationContext.Current;

            IObservable<TreeViewItem> treeViewItemExpanding =
                from dragEnterEvent in _dragEnter
                where SW.DragDropEffects.Scroll == (dragEnterEvent.EventArgs.Effects & SW.DragDropEffects.Scroll)
                let treeViewItem = GetTreeViewItem(dragEnterEvent.EventArgs)
                where treeViewItem != null
                from _ in
                    Observable
                        .Return(new Unit())
                        ...
                        .Until(
                            Observable.Merge(
                                _dragLeave
                                    .Select(dragLeaveEvent => GetTreeViewItem(dragEnterEvent.EventArgs))
                                    .Where(leaveTreeViewItem => leaveTreeViewItem == treeViewItem),
                                _drop
                                    .Select(dragLeaveEvent => GetTreeViewItem(dragEnterEvent.EventArgs))
                                    .Where(leaveTreeViewItem => leaveTreeViewItem == treeViewItem)))
                select treeViewItem;

            treeViewItemExpanding.Subscribe(treeViewItem => treeViewItem.IsExpanded = true);
        }
        ...
        protected override void OnDragEnter(SW.DragEventArgs args)
        {
            ...
            _dragEnter.OnNext(new Event<SW.DragEventArgs>(this, args));
        }
        ...
        protected override void OnDrop(SW.DragEventArgs args)
        {
            ...
            _drop.OnNext(new Event<SW.DragEventArgs>(this, args));
        }
        ...
        protected virtual Rect GetTreeViewItemRectExcludingChildren(TreeViewItem treeViewItem)
        {
            if (treeViewItem.IsExpanded)
            {
                ...
                if (rootVisual != null)
                {
                    ...
                    if (header != null)
                    {
                        ...
                        {
                            rectangle = new Rect(origin, header.RenderSize);
                        }
                        return rectangle;
                    }
                }
            ...
            return new Rect(0, 0, treeViewItem.RenderSize.Width, treeViewItem.RenderSize.Height);
        }
